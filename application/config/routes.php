<?php
defined('BASEPATH') OR exit('No direct script access allowed');
$route['default_controller'] = 'v1/health';
$route['404_override'] = 'errors/not_found';
$route['translate_uri_dashes'] = FALSE;
$route['v1/health'] = 'v1/health/index';
$route['v1/installations/villages'] = 'v1/installations/villages';
$route['v1/installations/enroll'] = 'v1/installations/enroll';
$route['v1/installations/auto-enroll'] = 'v1/installations/auto_enroll';
$route['v1/monitoring/summary'] = 'v1/monitoring/summary';
$route['v1/monitoring/villages'] = 'v1/monitoring/villages';
$route['v1/monitoring/rebind'] = 'v1/monitoring/rebind';
$route['v1/monitoring/catalog'] = 'v1/monitoring/catalog';
$route['v1/monitoring/branding'] = 'v1/monitoring/branding';
$route['v1/sync/pull'] = 'v1/sync/pull';
$route['v1/sync/push'] = 'v1/sync/push';
$route['v1/sync/ack'] = 'v1/sync/ack';
$route['v1/residents/verify'] = 'v1/residents/verify';
$route['v1/password-resets/request'] = 'v1/password_resets/request_code';
$route['v1/password-resets/complete'] = 'v1/password_resets/complete';
$route['v1/account-security/request'] = 'v1/password_resets/request_account_change';
$route['v1/account-security/complete'] = 'v1/password_resets/complete_account_change';
$route['v1/documents/(:any)'] = 'v1/documents/show/$1';
$route['v1/requests/(:any)/official-document'] = 'v1/requests/official_document/$1';
$route['v1/requests/(:any)/official-html'] = 'v1/requests/official_html/$1';
