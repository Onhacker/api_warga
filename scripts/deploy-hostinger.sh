...
printf 'Menjalankan migrasi database 006 sampai 029...\n'
migration_files=()
for migration in \
    006_service_catalog \
    ...
    015_sync_integrity \
    018_global_nik_uniqueness \
    019_monitoring_auth \
    020_reinstall_enrollment \
    021_installation_rebind_audit \
    022_global_service_catalog
do
...
migration_file="$API_REPO/database/migrations/029_local_letter_verification.sql"
if [[ ! -f "$migration_file" ]]; then
    printf 'ERROR: migrasi tidak ditemukan: %s\n' "$migration_file" >&2
    exit 1
fi
